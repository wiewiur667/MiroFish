  ...
  try {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Example: - select parent_601 (index 0): as representative of parent group...
        if (!headerMatch) {
        ...
        // Example: - **parent_601 (index 0)**: as representative of parent group...
        if (!headerMatch) {
        ...
        if (name) {
          ...
          // Continuation of reason (excluding end summary)
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '（该平台未获得回复）' || t === '(该平台未获得回复)' || t === '[无回复]' || t === '[No response]'
    }
    ...
    return () => h('div', { class: 'interview-display' }, [
      ...
      props.result.interviews.length > 0 && h('div', { class: 'interview-detail' }, [
        ...
        props.result.interviews[activeIndex.value]?.selectionReason && h('div', { class: 'selection-reason' }, [
          h('div', { class: 'reason-label' }, 'Reason'),
...
.section-number {
  ...
  font-size: 16px;
  color: #9CA3AF; /* dark gray, status-independent */
  font-weight: 500;
}
