  ...
  <div class="graph-panel">
    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <!-- Build/Simulation in progress hint -->
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">A small amount of content is still processing. Consider refreshing manually.</span>
...
const renderGraph = () => {
  ...
  // Count edges between each pair of nodes, collect self-loops
  tempEdges.forEach(e => {
  ...
  // Track how many edges have been processed per node pair
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    // Calculate curvature: spread apart for multiple edges, straight for single
    let curvature = 0
    if (totalCount > 1) {
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add central gravity to cluster isolated node groups to the center
  ...
  const getLinkPath = (d) => {
    ...
    // Perpendicular offset, calculated as ratio of distance, ensures curve visibility
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Detect if a real drag started (movement exceeds threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // On first real drag detection, restart simulation
          d._isDragging = true
  ...
  // Click on empty area to close details panel
  svg.on('click', () => {
    selectedItem.value = null
    node.attr('stroke', '#fff').attr('stroke-width', 2.5)
    linkGroup.selectAll('path').attr('stroke', '#C0C0C0').attr('stroke-width', 1.5)
